...
####把域名解析成ip
MNUM=`echo $MASTER|awk -F'.' '{print $3}'`
if ! expr $MNUM + 0 &>/dev/null;then
...
if [[ $SLAVE != ''  ]];then
	...
	for i in $SLAVETMP
	do
		SNUM=`echo $i|awk -F'.' '{print $3}'`
		if ! expr $SNUM + 0 &>/dev/null;then
...
if [[ $MASTER != '' && $TYPE == 'uninstall'  ]];then
	...
	ssh $MASTER "mkdir -p /data/install/"
