...
def  redis_status():
    ...
    #redis_host=[{'hostname':'rdb1v.infra.bjza.pdtv.it','port':6084}]
    cur_time=get_hour()
    if len(redis_host)==0:
        rt={}
        rt['status']=-1
	...
	try:
		...
		max_memory=redis_conn.config_get('maxmemory').get('maxmemory',0)
	except Exception,e:
		print traceback.print_exc()
		continue
        if len(redis_info)==0:
            ...
            continue
        ops=redis_info['total_commands_processed']
        used_memory=redis_info['used_memory']
        used_memory_rss=redis_info['used_memory_rss']
        aof_size=int(redis_info['aof_current_size']/1024/1024)
        mem_frag=int(redis_info['mem_fragmentation_ratio'])
	max_memory=int(max_memory)
	mem_pct=int(used_memory*100/max_memory)
        sql="replace into cmdb.redis_hour_ops(hostname,port,day_time,ops,used_memory,used_memory_rss,mem_frag,max_memory,mem_pct,aof_size) values('%s',%d,'%s',%d,%d,%d,%d,%d,%d,%d)"%(hostname,int(port),cur_time,ops,used_memory,used_memory_rss,mem_frag,max_memory,mem_pct,aof_size)
