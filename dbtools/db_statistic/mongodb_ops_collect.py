...
def get_all_port():
    try:
        ...
        sql='select  hostname,port from cmdb.mongodb_ins group by port;'
...
def collect_metric_data(host,port):
	rt={}
	mongodb_metric_list={}
	rep_name='repl_%d'%int(port)
	client=pymongo.MongoClient(host,port,replicaSet=rep_name)
	db=client['admin']
	db.authenticate('admin','pandatv#>!$999')
	m_rt=db.command(pymongo.son_manipulator.SON([('serverStatus', 1)]))
	m_rt=m_rt['opcounters']
	rt['status']=0
	rt['result']=m_rt
	return rt
...
def main():
	...
	#all_port=[{'hostname':"db4v.infra.bjac.pdtv.it","port":3306}]
	    ...
	    per_com_rt=temp_ops
            cur_time=get_hour()
	    sql="replace into cmdb.mongodb_hour_ops(hostname,port,day_time,total_command,`select_ops`,`insert_ops`,`update_ops`,`delete_ops`) values('%s',%d,'%s',%d,%d,%d,%d,%d)"%(hostname,int(port),cur_time,per_com_rt['command'],per_com_rt['query'],per_com_rt['insert'],per_com_rt['update'],per_com_rt['delete'])
	    print sql
            conn=get_conn(user=admin_user,passwd=admin_passwd,host=admin_host,port=admin_port)
            conn=conn['value']
            conn.autocommit(1) 
