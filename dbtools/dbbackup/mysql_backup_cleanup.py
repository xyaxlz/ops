...
def get_remains(host, backup_items):
    try:
        if not default_port or not backup_items:
            return REMAINS
        sql = """ select prod_info, keep_days as backup_remains from cmdb.db_backup_policy as a left join cmdb.mysql_ins  as b on  a.host=b.ip  where b.prod_info='%s' limit 1;""" % (backup_items['tag'])
    ...
    try:
        ...
        for period_dir in period_dirs:
            if os.path.exists(period_dir) is True:
                ...
                if tmp_dirs:
                    mysqlport_dirs.extend(['%s/%s' % (period_dir, i) for i in tmp_dirs])
        walk_result = {}
        for mysqlport_dir in mysqlport_dirs:
            if os.path.isdir(mysqlport_dir) is True:
                tmp_dirs = os.listdir(mysqlport_dir)
                ret = get_port_period(mysqlport_dir)
                walk_result[mysqlport_dir] = {'period': ret['period'], 'tag': ret['tag'], 'backups': tmp_dirs}
        return walk_result
    except Exception, e:
...
def get_truncate_backup(backup_dir, backup_items):
    try:
	print backup_items
        remains = get_remains(default_port, backup_items)
...
def main():
    try:
        log('start mysql backups cleaning...')
